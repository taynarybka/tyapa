...
const HEALTH_DEMO_FALLBACK = true;
const MOVE_COST = 10;
...
const MESSAGE_VARIANTS = {
  ...
  demoStepsGranted: [
    () => "Демо-шаги включены. Можно обновить шаги за сегодня.",
  ],
  stepsEarned: [
...
let demoHealthProviderInstance = null;
...
function demoHealthStepsForDate(dateKey) {
  let hash = 0;
  for (const char of dateKey) hash = (hash * 31 + char.charCodeAt(0)) % 100000;
  return 4200 + (hash % 9400);
}

function demoHealthProvider() {
  if (demoHealthProviderInstance) return demoHealthProviderInstance;

  demoHealthProviderInstance = {
    isDemo: true,
    async requestPermission() {
      const allowed = window.confirm(
        "Разрешить Тяпе подключиться к тестовым данным здоровья для проверки шагов за сегодня и последние 7 дней?",
      );
      return allowed ? "granted" : "denied";
    },
    async getTodaySteps() {
      return demoHealthStepsForDate(todayKey());
    },
    async getStepsBetween(startIso, endIso) {
      const start = parseStoredDate(startIso);
      const end = parseStoredDate(endIso);
      ...
      while (cursor < end) {
        ...
        if (overlapEnd > overlapStart) {
          const daySteps = demoHealthStepsForDate(dateKeyFromDate(dayStart));
          total += Math.round(daySteps * ((overlapEnd - overlapStart) / (dayEnd - dayStart)));
        }
  ...
  return demoHealthProviderInstance;
}

function activeStepProvider() {
  if (STEP_SOURCE_MODE === "manual") return null;
  return nativeStepProvider() || (HEALTH_DEMO_FALLBACK ? demoHealthProvider() : null);
}
...
function tutorialIsSuppressed() {
  return Boolean(introCutsceneActive || shellHomeOpen || state.familyOverlay || state.activeDialogue);
}
...
async function grantSteps() {
  ...
  if (state.stepsPermission === "granted") {
    if (provider?.isDemo) {
      setMessage("Тестовое подключение к здоровью включено. Проверяю шаги за 7 дней.");
    } else if (provider) {
      setMessage("Доступ к шагам включен.");
    } else {
      setRandomMessage("demoStepsGranted");
    }
    playSfx("toggleOn");
    settleStepsOnEntry({ silent: false, source: "permission" });
  } else {
...
function render() {
  ...
  const isManualSteps = STEP_SOURCE_MODE === "manual";
  elements.permissionStatus.textContent = isManualSteps
    ? "ручной демо-ввод"
    : state.stepsPermission === "granted"
      ? (hasNativeSteps ? "доступ выдан" : "тестовый доступ выдан")
      : "не выдан";
  elements.permissionPanel.hidden = isManualSteps || state.stepsPermission === "granted" || !provider;
  elements.stepsPanel.hidden = !isManualSteps;
  elements.grantStepsButton.textContent = isManualSteps
    ? "Включить демо-шаги"
    : hasNativeSteps ? "Подключить Здоровье" : "Подключить Здоровье (тест)";
...
function startMapDrag(event) {
  if (shellHomeOpen || state.seedMode || state.familyOverlay || state.activeDialogue || event.target.closest(".zoom-controls")) return;
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) {
    ...
    settleStepsOnEntry({ silent: true, source: "resume" });
    if (audioSettings.musicEnabled && audioContext) {
      startMusic({ persist: false });
    }
  }
